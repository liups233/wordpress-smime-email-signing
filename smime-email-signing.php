 ...
 * Plugin Name: S/MIME Email Signing
 * Plugin URI: https://github.com/liups233/wordpress-smime-signing
 * Description: A plugin for signing all outbound emails with S/MIME certificate. 
 * Version: 1.0
 * Requires PHP: 7.2
 * Author: Liups233
 ...
 * Text Domain: smime-email-signing
 */
...
class SMIME_EMAIL_SIGNING {
	...
	public static function sanitize_settings( $input ) {
		$output = array();
		if ( isset( $input['smime_public_key'] ) ) {
			$output['smime_public_key'] = sanitize_text_field( $input['smime_public_key'] );
		}
		if ( isset( $input['smime_private_key'] ) ) {
			$output['smime_private_key'] = sanitize_text_field( $input['smime_private_key'] );
		}
		if ( isset( $input['smime_password'] ) ) {
			$output['smime_password'] = sanitize_text_field( $input['smime_password'] );
		}
		if ( isset( $input['smime_cert_chain'] ) ) {
			$output['smime_cert_chain'] = sanitize_text_field( $input['smime_cert_chain'] );
		}
		return $output;
	}
